...
void loop()                                      // run over and over again
{
  ...
  if(Rfid_value == "TN11 20210011429")
  {
    ...
    if(FingerPrintID==4)
    {
      ...
      while(1)                             //Driver is drunk don't proceed further
      {
      analogWrite(8,20);
      delay(1000);
      analogWrite(8,LOW);
  ...
  else if(Rfid_value == "TN91 20210003462")
  {
    ...
    if(FingerPrintID == 1)
    {
      ...
      float Alcoholcontent = GasSensor();                              // call to read alcohol content from gas sensor
      Serial.print("Alcoholcontent : ");
      Serial.println(Alcoholcontent); 
      if(Alcoholcontent > 1.99)
      {
      digitalWrite(Gas_off,HIGH);
      lcd.clear();
